...
def tcp_connect():
	print('created the socket id '+ judge2 +' Successfully!')
...
# receive from cloud, and send to tun
def worker_thread1(tun):
	print("forward from ", address ,"to", tun.name)
	while(True):
		...
		if '+CSONMI: 0' in str(rec_buff.decode()):
			...
			tun.write(message_bytes)

# receive from tun, and send to cloud
def worker_thread2(tun):
	print("forward from ", tun.name,"to", address)
	while(True):
		data = tun.read(512)
		data_hex = data.hex()
		if data:
...
def main():
	try:
		...
		time.sleep(0.5)
		tun = createTun()
		t1 = threading.Thread(target=worker_thread1,args=[tun])
		t2 = threading.Thread(target=worker_thread2,args=[tun])
		t1.start()
		t2.start()
		t1.join()
		t2.join()
